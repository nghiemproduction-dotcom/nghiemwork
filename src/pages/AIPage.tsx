...
export default function AIPage() {
  ...
  const [mentionMode, setMentionMode] = useState<'none' | 'task' | 'group'>('none');
  ...
  const executeAction = useCallback((action: AIAction): string => {
    switch (action.type) {
      ...
      case 'ADD_TEMPLATE': {
        ...
        // AI always creates single templates; groups are built manually from singles
        if (action.subtasks && action.subtasks.length > 0) {
          // Create each subtask as a separate single template
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 pb-20 pt-2 glass-strong border-t border-[var(--border-subtle)]">
        ...
        {mentionMode !== 'none' && (
          <div className="mt-2 rounded-xl bg-[var(--bg-elevated)] border border-[var(--border-subtle)] p-2 max-h-40 overflow-y-auto">
            <p className="text-[10px] text-[var(--text-muted)] mb-1">
              {mentionMode === 'task'
                ? 'Chọn VIỆC ĐƠN để chèn vào câu (sử dụng @TênViệc).'
                : 'Chọn NHÓM VIỆC để chèn vào câu (sử dụng @@TênNhóm).'}
            </p>
              ...
              {mentionMode === 'task'
                ? singleTaskOptions.map(t => (
                    <button
                      ...
                      className="w-full text-left px-2 py-1 rounded-lg text-[11px] text-[var(--text-primary)] hover:bg-[var(--bg-surface)] active:bg-[var(--accent-dim)] truncate"
                    >
                      {t.title}
                    </button>
                  ))
                : groupTemplateOptions.map(g => (
                    <button
                      key={g.id}
                      onClick={() => insertMention(g.title)}
                      className="w-full text-left px-2 py-1 rounded-lg text-[11px] text-[var(--text-primary)] hover:bg-[var(--bg-surface)] active:bg-[var(--accent-dim)] truncate"
                    >
                      {g.title}
                    </button>
