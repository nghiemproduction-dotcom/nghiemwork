...
type StatsView = 'today' | 'week' | 'finance';
...
export default function StatsPage() {
  ...
  const stats = useMemo(() => {
    ...
    const totalTime = tasks.filter(t => t.status === 'done' && t.duration && !t.parentId).reduce((sum, t) => sum + (t.duration || 0), 0);
    ...
    <div className="flex flex-col h-full px-4 pt-4 pb-24 overflow-y-auto">
      ...
      <div className="flex gap-1.5 mb-4">
        {[
          ...
          <button
            key={tab.key}
            onClick={() => setView(tab.key as StatsView)}
