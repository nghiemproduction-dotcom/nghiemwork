    ...
    <div className="fixed inset-0 z-[95] flex items-end sm:items-center justify-center bg-black/70" onClick={onClose}>
      <div className="w-full max-w-lg max-h-[90vh] bg-[var(--bg-elevated)] rounded-t-2xl sm:rounded-2xl overflow-hidden flex flex-col animate-slide-up"
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          ...
          {/* Group */}
          {groupTemplate && (
            <div className="flex items-center justify-between gap-2 px-3 py-2.5 rounded-xl bg-[var(--bg-surface)] border border-[rgba(0,229,204,0.10)]">
              <div>
                <p className="text-[10px] text-[var(--text-muted)]">Nhóm việc</p>
                <p className="text-sm font-medium text-[var(--text-primary)]">{groupTemplate.title}</p>
              </div>
              {(() => {
                const groupTasks = tasks.filter(t => !t.parentId && (t.groupTemplateId || ((t.templateId && templates.find(x => x.id === t.templateId)?.templateType === 'group') ? t.templateId : undefined)) === effectiveGroupId);
                const total = groupTasks.length;
                const done = groupTasks.filter(t => t.status === 'done').length;
                const percent = total > 0 ? Math.round((done / total) * 100) : 0;
                return (
                  <div className="text-right">
                    <p className="text-xs font-bold text-[var(--accent-primary)] tabular-nums">{percent}%</p>
                    <p className="text-[10px] text-[var(--text-muted)]">{done}/{total} việc đơn</p>
                  </div>
                );
              })()}
            </div>
            ...
            <div>
              <p className="text-xs text-[var(--text-muted)] mb-1.5 flex items-center gap-1">
                ...
                {(() => {
                  ...
                  return <>Việc đơn ({doneCount}/{subtasks.length} • {percent}%)</>;
                })()}
              </p>
